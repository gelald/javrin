...
![image-20200928225821932](https://gitee.com/ngyb/pic/raw/master/image-20200928225821932.png)
...
- 那么当要读入的数据页不在Buffer Pool中时，就需要在Buffer Poo中申请一个新的数据页，但Buffer Poo中数据页是一定的，当数据页达到上限时此时就需要把最久不使用的数据页从内存中淘汰掉。**但如果淘汰的是脏页，那么就需要把脏页刷到磁盘里(刷脏)才能进行复用。**
