...
![B Tree 分裂](https://wingbun-notes-image.oss-cn-guangzhou.aliyuncs.com/images/20230112155002.png)

- 合并：当一个节点发生分裂而产生一个父级节点，且原父级节点已有关键字，那么这两个节点可以根据不超过最大度数的规则来合并称为一个父级节点

![B Tree 合并](https://wingbun-notes-image.oss-cn-guangzhou.aliyuncs.com/images/20230112155705.png)
...
- **不足**：B Tree 在数据插入与删除的时候，会破坏 B Tree 自身的平衡，不得不使用合并与分裂来保证平衡，**由于 B Tree 节点不仅存储索引也存储数据区，所以导致合并、分裂结点的操作效率不高**，在节点数量较多的情况下性能影响大；并且所有节点都存储数据，会导致数据查询的时间不稳定。
...
![B+ Tree 结构](https://wingbun-notes-image.oss-cn-guangzhou.aliyuncs.com/images/20230113214156.png)
...
4. 度数更多，B+ Tree 的非叶子结点不需要存储数据，只需要存储索引和指针，一个非叶子节点能存放更多的索引，从而让树的高度更低，进而减少 IO 次数

#### 三层 B+ 树大约能存多少数据

- 假设主键是 BIGINT 类型，需要占用 8 个字节，指针需要占用 6 个字节，所以一个非叶子节点需要占据 14 个字节
- InnoDB 一页数据的大小是 16KB = 16384 字节
- 那么一个数据页大约可以存 1170 个非叶子节点
- 第一层是一页数据，并携带有 1170 个索引（指针）
- 第二层是 1170 页数据，其中每一页携带有 1170 个索引（指针）
...
- 二级索引：除了聚簇索引外，其他所有索引都是二级索引。二级索引中的非叶子结点存储的是索引，叶子结点存储的是对应数据行的主键，因此通过二级索引查询数据时需要进行回表，涉及到两次查找。
...
- 一般选择在 where 条件、order 排序、join 连接、group by 分组的字段上建立索引，因为这里**涉及到数据的查询与排序**，可以充分发挥 B+ Tree 的作用。

- 索引的数量不宜过多，一个表应该**尽可能精简索引的数量**，因为一个索引对应一棵 B+ Tree，索引太多会浪费空间，插入删除数据时可能涉及 B+ Tree 的分裂合并操作，导致更新索引树效率低。

- **如果索引字段较长，应该建立前缀索引**。因为索引字段太长会导致占用的空间太大，进行索引匹配时也需要进行更长的时间；此外 B+ Tree 上一页数据能存储的索引关键字的数量也不多，导致度数也少，进而导致 B+ Tree 的深度变得很大。前缀索引创建的计算方式：[mysql索引之前缀索引\_ITPUB博客](http://blog.itpub.net/30317998/viewspace-2654308/)

- **索引值应该足够稳定**，频繁更新的字段不适合做索引，因为索引数据频繁地更新，B+ Tree 需要保持平衡而进行结点的分裂和合并、重新排序，这个调整的过程是需要耗费性能的。如果这一行数据插入后，索引对应的字段一直不会被修改，这种情况是最好的。
...
- **随机无序的值不适合做索引**，比如 UUID 和身份证这种随机且无序的值，因为 B+ Tree 的索引是有序组织的，所以无序的数据插入删除也就意味着需要频繁的分裂与合并操作。
...
比如上面例子中创建了 `name` 和 `age` 的联合索引，那么具体的索引结构会先按 `name` 排序，后按 `age` 排序
...
- **模糊查询时使用 like % 前缀**。原因和 B+ Tree 索引结构有关系，索引关键字都是从左往右排布的，当前缀未知的时候，就无法定位索引关键字，因此查询时只能进行全表扫描。
