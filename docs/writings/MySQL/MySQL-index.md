...
**劣势**：当出现大量重复键时，存在**哈希冲突**；**数据排序、模糊查询难以实现**
...
B 树最大的特点是一个节点的度数 = 一个节点的关键字数 + 1，关键字数越多，度越多，树深度越小，IO 次数越少
...
B+ 树中的节点，在 MySQL 中代表的是 Page 数据页，所以 B+ 树中一个节点的大小是 16 KB，非叶子节点存储索引数据和下一个节点的地址，因此 B+ 树的深度和度数由索引数据大小来决定

![](https://wingbun-notes-image.oss-cn-guangzhou.aliyuncs.com/images/20230112161207.png)

B+ 树特点：

1. B+ 树节点的关键字和度数的关系是 1 : 1
2. B+ 树只有叶子节点存储数据，其他节点都只存储关键字索引
3. B+ 树的叶子节点都会有指向上一个和下一个叶子节点的指针，形成一个有序双向链表，方便做**范围查询**
...
1. **磁盘 IO 次数更少**，由于 B+ 树的非叶子节点仅存储索引，不存储数据，所以**一个非叶子节点能存储更多的索引，从而让树的高度更低**
...
**在 MyISAM 存储引擎中，聚簇索引和二级索引在结构上没有区别，叶子节点统一存储的是数据的磁盘地址，只是主键索引要求关键字是唯一的**
...
- 聚簇索引：聚簇索引中索引键值的逻辑顺序和表数据行的物理顺序是一致的。聚簇索引中非叶子节点存储的是索引关键字，叶子节点存储的是数据。InnoDB 中索引和数据都存储在一个文件中，是因为数据是通过聚簇索引来组织表的，InnoDB 中的主键索引就是聚簇索引。
- 二级索引：除了聚簇索引外，其他所有索引都是二级索引。二级索引中的非叶子节点存储的是索引，叶子节点存储的是对应数据行的主键，因此通过二级索引查询数据时需要进行回表，涉及到两次查找。
...
索引覆盖的概念是：如果需要查询的字段列只包含联合索引列中的字段，那么在普通索引树上查找到具体的索引数据后，因为节点上包含了联合索引中的多个索引数据，从这个节点上取出所需字段数据即可，无需回表。索引覆盖往往和联合索引一起使用，因为单一索引没有索引覆盖的意义。

例如，有一个联合索引 `index(f1, f2, f3)` ，此时使用这个 SQL 语句：`select f1, f2, f3 from table1 where f1 = 'a'` ，那么只需要执行一次 B+ 树的树查找，不需要再次回表。
