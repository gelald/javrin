...
- **当老年代空间被填满后，会触发 Major GC（也称 Old GC），Major GC 通常会花费更多的时间**
...
程序计数器是线程私有的一块小内存，可以看作是当前线程所执行的字节码的**行号指示器**，分支、循环、跳转、多线程恢复都依赖它
...
> 在 HotSpot 虚拟机中，虚拟机栈和本地方法栈是同一个，没有区分
...
- 新生代：使用复制算法，高效回收短生命周期的对象
- 老年代：使用标记整理算法，回收长生命周期的对象

### Survivor 区为什么要分为两个

**为了避免内存碎片**，根据**复制算法**的要求，必须有一个 `From` 区、一个 `To` 区，在执行回收时，将存活对象复制到 `To` 区，然后清空 `From` 区，而后两块区域角色互换

### 栈内存发生 OOM 的情况

- 方法递归太深，栈帧数量超出栈的限制，会发生 `StackOverflowError`
- 没有更多的内存空间创建新的线程（线程太多），会发生 `OutOfMemoryError`
