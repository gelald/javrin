...
垃圾回收一般回指**堆内存**的垃圾回收，简单介绍一下方法区的回收后重点讲解堆内存的垃圾回收
...
```mermaid
flowchart TD
    Start[新对象创建] --> CheckSize{对象大小？}

    CheckSize -->|大对象<br/>>PretenureSizeThreshold| OldDirect[直接进入老年代]
    CheckSize -->|普通对象 | Eden[尝试 Eden 分配]

    Eden --> EdenFull{Eden 满？}
    EdenFull -->|否 | Success[分配成功]
    EdenFull -->|是 | MinorGC[触发 Minor GC]

    MinorGC --> AfterGC{Eden 有空间？}
    AfterGC -->|是 | Success
    AfterGC -->|否 | CheckOld{老年代有空间？}

    CheckOld -->|是 | FullGC[触发 Full GC]
    CheckOld -->|否 | OOM[OutOfMemoryError]
...
> **面试话术**：对象进入老年代有四种情况——**大对象直接进入**、**长期存活**（年龄≥15）、**动态年龄判断**、**空间分配担保失败**。
...
```mermaid
flowchart TD
    MinorGC[Minor GC 前] --> Check[检查老年代空间]

    Check --> Condition{老年代可用空间<br/>≥新生代总大小？}

    Condition -->|是 | Safe[允许 Minor GC]
    Condition -->|否 | CheckAvg[检查历史平均晋升大小]

    CheckAvg --> AvgCheck{老年代可用空间<br/>≥平均晋升大小？}

    AvgCheck -->|是 | Safe
    AvgCheck -->|否 | FullGC[触发 Full GC]

    style Safe fill:#90EE90
    style FullGC fill:#FFB6C1
```
