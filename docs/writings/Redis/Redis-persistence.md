...
> **追问：AOF 重写期间有新写入怎么办？**
>
> fork 之后的新写命令会同时记录到**重写缓冲区**。子进程完成重写后，主进程将重写缓冲区的增量命令追加到新 AOF 文件末尾，保证数据不丢失。

### 4.4 AOF 重写不是"读取旧 AOF 再压缩"

这是一个常见误解。**AOF 重写是直接遍历当前内存数据，生成最精简的命令**，而不是去读取和压缩旧 AOF 文件。

> 比如 k1 变量在 AOF 文件中 SET 了 100 次，但是重写时会直接从内存快照中获取最新值，并仅生成一条 SET 命令
...
Redis 4.0 引入，将 RDB 和 AOF 的优势结合：**AOF 重写时，先写入 RDB 格式的全量数据(仅包含 fork 那一刻的数据)，再追加 AOF 格式的增量命令(重写期间新数据追加)**。

下面通过时间线流程，展示混合持久化模式下 AOF 重写的完整过程：

```mermaid
flowchart TB
    START[触发 BGREWRITEAOF] --> FORK["主进程 fork 子进程<br/>子进程获得 12:00 内存快照"]

    FORK --> CHILD["子进程<br/>遍历内存, 写入 RDB 格式全量"]
    FORK --> MAIN["主进程<br/>继续处理命令, 攒入重写缓冲区"]

    CHILD --> CHILD_DONE["12:02 子进程完成"]
    MAIN --> RWBUF["重写缓冲区<br/>12:00~12:02 增量命令"]

    CHILD_DONE --> MERGE[主进程将重写缓冲区<br/>追加到新文件末尾]
    RWBUF --> MERGE

    MERGE --> FILE[新混合 AOF 文件]
    FILE --> P1["RDB 格式（二进制）<br/>12:00 内存全量<br/>fork 时刻的静态照片"]
    P1 --> P2["AOF 格式（文本）<br/>12:00~12:02 增量命令<br/>重写期间的动态补丁"]

    P2 --> REPLACE[rename 原子替换旧文件]
    REPLACE --> DONE[后续写命令继续追加到文件末尾]
```
...
RDB 的 `bgsave`、AOF 的 `bgrewriteaof` 都依赖 `fork()` 创建子进程。`fork()` 是操作系统提供的系统调用，创建一个子进程：
...
```
# 扫描大 Key（Redis 4.0+） bash 命令
redis-cli --bigkeys

# 查看指定 Key 的内存占用（Redis 4.0+） redis 命令
